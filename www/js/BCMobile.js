...
//    serverPrefix: "https://ulxlcqmkzk.localtunnel.me/",
...
function cameraFailure(errMsg) {        // general failure - though usually the cancel/back button...
    console.log("Camera error - "+errMsg);
//    toast("Camera error - "+errMsg);
}
/*
function onErrorCreateFile(err) {
    console.log("onErrorCreateFile failure");
}
function onErrorResolveUrl(err) {
    console.log("onErrorResolveUrl failure");
}
...
*/
    getSelfie: function (srcType) {
        ...
        navigator.camera.getPicture(
            ...
            //    destinationType: Camera.DestinationType.NATIVE_URI,
                sourceType: srcType,
                encodingType: Camera.EncodingType.JPEG,     // since from library will come that way anyway...
    ...
    initialize: function () {
        ...
        //    event.preventDefault();
            console.log("btnTakeSelfieSnap");
            takeSelfiePage.getSelfie(Camera.PictureSourceType.PHOTOLIBRARY);        // or SAVEDPHOTOALBUM
        });
...
function fUploadImage(endPoint, params, fSuccess ) {
    var imageURI = BCApp.fileUrl;
    var srvrEndPoint = BCApp.serverPrefix + endPoint;
    ...
    ft.upload(imageURI, srvrEndPoint, function(resp){
        jData = JSON.parse(resp.response)
        fSuccess(jData);
    }, function(error){
        console.log("Image processing hard error - " + error.code);
        BCApp.serverError = "Image processing Server error - " + error.code;
        if (endPoint==="selfie2Palette")
...
var submitSelfie = {
    ...
    */
    ...
    doTheSubmit: function () {
        ...
        params.sku = BCApp.sku;
        fUploadImage("selfie2Palette", params, 
            function(jData) {
                console.log("doTheSubmit success - from server: "+jData.good);
                if (jData.good) {
...
/*
        var imageURI = BCApp.fileUrl;
        var srvrEndPoint = BCApp.serverPrefix + "selfie2Palette";

        var options = new FileUploadOptions();
        options.fileKey = "image";
//        options.fileName = imageURI.substr(imageURI.lastIndexOf('/') + 1);
        options.fileName = "Selfie.jpg";
        options.mimeType = "image/jpeg";
        ...
        ft.upload(imageURI, srvrEndPoint, function(resp){
            ...
            console.log("Selfie2Palette hard error - " + error.code);
            BCApp.serverError = "Server error - " + error.code;
            $("body").pagecontainer("change", "#pgBadResults", {}); // change page
        }, options);
...
var takeImagePage = {
    ...
    doTheSubmit: function () {
        ...
        var params = new Object();
        params.sku = BCApp.sku;
        params.palLookup = BCApp.jPalPayload.palLookup;
//        params.palLookup = blobifyImage(BCApp.jPalPayload.palLookup, "image/png");
        params.palList = "not used";
//        params.palList = BCApp.jPalPayload.palList;
        params.palId = BCApp.jPalPayload.palId;
        fUploadImage("evalImage", params, 
            function(jData) {
                console.log("doTheSubmit success - from server: "+jData.good);
                loading(false);  // hide spinner
        ...
        */
        /*
        var imageURI = BCApp.fileUrl;
        var srvrEndPoint = BCApp.serverPrefix + "evalImage";

        var options = new FileUploadOptions();
        options.fileKey = "image";
        options.fileName = "Original.jpg";
        options.mimeType = "image/jpeg";
        console.log(options.fileName);

        var params = new Object();
//        params.palLookup = JSON.stringify( blobifyImage(BCApp.jPalPayload.palLookup, "image/png") );
//        params.palList = [];
//        params.palList = BCApp.jPalPayload.palList;
//        params.palId = BCApp.jPalPayload.palId;
        params.sku = BCApp.sku;
...
//        options.params = params;
        ...
        var ft = new FileTransfer();
        ft.upload(imageURI, srvrEndPoint, 
            function(resp) {
                jData = JSON.parse(resp.response)
                console.log("success - read from web:"+jData.good);
                loading(false);  // hide spinner
...
//        BCApp.dataUrl = "data:image/png;base64," + dataUrl;
        BCApp.fileUrl = "";
        BCApp.dataUrl = dataUrl;

        $('#imgImageUploadedImg').attr("src", "data:image/png;base64," + dataUrl);
        $('#dvImageFileTake').hide();
        $('#dvImageFileSpecd').show();
        loading(true);  // show spinner

        takeImagePage.doTheSubmit();
...
var validateResults = {
    initialize: function () {
        ...
        navigator.camera.cleanup(cleanupSuccess, cleanupFailure);   // original image no longer required
