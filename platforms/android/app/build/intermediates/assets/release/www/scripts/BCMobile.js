...
var BCApp = {
    ...
    serverPrefix: "http://bestcolors-cf8ef05e.6854d846.svc.dockerapp.io:80/",       // Service is long lived...
    ...
    initialize: function () {           // Called when DOM is ready, first time only
        ...
        */
        ...
        $('#numVisits').text("visit: " + BCApp.jPersist.visits);
    //    toast("visit: " + BCApp.jPersist.visits);

        fdbkEmail = "mailto:" + BCApp.mailTo + "?subject=Best%20Colors%20Feedback-" + BCApp.sku; // + ";" + device.model + ";" + device.version;
        $('#vwPalMailto').attr("href", fdbkEmail);
        $('#vwImgMailto').attr("href", fdbkEmail);
        $('#vwMreInfoMailto').attr("href", fdbkEmail);
...
var viewPalette = {
    initialize: function () {
        ...
        $("#btnUseMyColors").on('click', function () {
            console.log("btnUseMyColors");
            BCApp.backToParent(true);
        });
    }
};
...
$(document).on('pause', BCApp.fOnPause);                // for phonegap
